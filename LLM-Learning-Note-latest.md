...
#### DeepSeek 稀疏注意力（DeepSeek Sparse Attention, DSA）——NSA 的细粒度深化形态

> 定位一句话：DSA 可以视作“在 MLA (MQA 模式) 之上，以极轻量可学习索引器 + 精细 Top-k Token 选择”对 NSA“分层稀疏（压缩/选择/窗口）范式”中“选择+精细建模”部分的进一步下钻与工程化极致优化；它把 NSA 的“结构性块级 + 可学习重要性”推进为“Token 级实时检索”，在长上下文下获得更高性价比。

##### 1. 设计动机与与 NSA 的关系概览
NSA：三路（压缩 / 选择 / 窗口）→ 结构层级稀疏，选取的是“块 + 局部 + 少量精细 Token”。
DSA：引入“闪电索引器 (Lightning Indexer)”对所有历史 Token 做一次极轻量打分，再精确抽取 Top-k 真实 KV 参与主注意力，形成“先轻量索引、后精算注意”的二段式流水。二者关系：
- 颗粒度：NSA 的 Selected 分支常以块/候选池+Top-K；DSA 直接以 Token 粒度全局打分。
- 架构绑定：DSA 原生绑定 MLA（且使用其 MQA 变体），充分共享潜在 KV 表示以提升缓存/带宽效率；NSA 更偏抽象范式，可与 MHA / MLA 搭配。
- 计算路径：DSA 的“索引 → 主注意”两级，强化了 Selected 分支；压缩与窗口在 DSA 初版中由 MLA + 稀疏 Top-k 的高召回设计间接弥补。
- 组合潜力：未来可出现 “NSA(宏观块稀疏) + DSA(块内精细 Token 级再稀疏)” 的层级嵌套，形成二层检索（Block Pruning → Token Pruning）。

##### 2. 核心组件：闪电索引器 (Lightning Indexer)
索引器目标：对当前查询 token h_t 与其之前的每个 token h_s 生成一个“索引分” I_{t,s}，用于后续 Top-k 选取。其计算公式（论文公式 (1)）如下：

公式 (1)：
![公式1 闪电索引器打分](DSA公式/1.png)

特征要点：
1. 少头数 (H_I 很小) + 低维投影 (d_I << d) + ReLU 激活 → 极低算子常数，可用 FP8 部署。
2. 查询侧派生 (q_{·,j}, w_{·,j})，历史侧派生 k；按头聚合形成可加权的 index score。
3. 复杂度形式仍 O(L^2)，但常数远低于主注意力（尤其与 MLA 的主干 KV 操作相比）。

##### 3. 精细 Token 选择与主注意力融合
索引器输出全序列打分后，仅保留 Top-k 个 KV 条目进入“真正的”注意力计算。注意力输出 (论文公式 (2))：

公式 (2)：
![公式2 稀疏注意力输出](DSA公式/2.png)

说明：与传统“稠密 softmax over 全序列”不同，这里主注意力只在选中集合上进行归一化与加权，K 远小于 L，主注意力 FLOPs 下降为 O(L·K)。

##### 4. 在 MLA (MQA 模式) 下的实例化
为什么绑定 MLA 的 MQA 模式？
1. MQA 共享一份 KV → 选中 Top-k 后，所有 Query 头复用同一组 KV，访存最小。
2. MLA 的潜在表示进一步压缩 KV 物理尺寸，为“索引 + 稀疏主注意”的二级访问再降带宽。
3. Kernel 级别：一个 KV entry 可被所有 Query 头重复利用（满足 GPU 上算子融合与块复用优化需求）。

架构示意（论文图，绿色路径为索引器，公式对应 1–2）：
![图 DSA 架构示意](DSA公式/5.png)

##### 5. 训练两阶段：从稠密到稀疏的平滑迁移
为避免直接稀疏化导致能力震荡，采用“稠密热身 → 稀疏联合优化”连续训练（Continued Training）。

Stage A：Dense Warm-up（仅训练索引器，主模型保持稠密注意力 & 冻结）
- 目标：让索引器分布对齐原主注意力的多头聚合分布。
- 做法：对 t 时刻，先把所有注意力头的分数求和并 L1 归一化得到目标分布 p_{t,:}。
- 优化：最小化索引器输出分布与 p_{t,:} 的 KL 散度（论文公式 (3)）。

公式 (3)：
![公式3 Warm-up KL 损失](DSA公式/3.png)

超参：学习率 1e-3；1000 步；每步 16 × 128K tokens，总 2.1B tokens。

Stage B：Sparse Training（引入 Top-k，主模型 + 索引器联合）
- 引入选择集 S_t = { s | I_{t,s} ∈ Top-k(I_{t,:}) }。
- 主模型：只看选中的 KV 做语言建模；索引器：继续对齐但仅在 S_t 上对齐（聚焦排序质量）。
- 训练图截断：索引器输入与主模型梯度分离（detach），避免双向干扰；主模型只受语言建模损失驱动，索引器只受 KL 损失 L_I 驱动。
- 损失形式（论文公式 (4)）：

公式 (4)：
![公式4 稀疏阶段 KL 损失](DSA公式/4.png)

关键配置：学习率 7.3e-6；k = 2048；15000 步；每步 480 × 128K tokens，总 943.7B tokens。

对比 NSA：NSA 的稀疏拓扑在结构上预定义三路；DSA 在这一阶段“学习一个全局可适配的 Token 集合”，与 NSA 的 Selected 分支功能类似但粒度更细、检索路径更短。

##### 6. 后训练（Post-Training）保持可比性
为隔离“架构变化”带来的纯粹影响，DSA 版本沿用 Terminus 相同的后训练流程：
1. Specialist Distillation（专家蒸馏）：多域专家（数学 / 编程 / 推理 / Agent 编排 / 搜索 等）生成高质量数据再蒸馏统一模型。
2. Mixed RL (GRPO)：将“推理 + Agent + Alignment”合并为单阶段 RL；奖励含结果正确性、长度惩罚、语言一致性、生成式评估 Rubrics。

##### 7. 训练稳定性与效率观察
报告要点：
- RL 期间（BrowseComp / SWE Verified）准确率曲线与 Terminus 高度重合 → 稳定。
- Prefill / Decode 均降本：主注意力 O(L·K)；索引器 O(L^2) 但常数极低（低维 + 少头 + FP8 + ReLU）。
- 短上下文 Prefill：可用 Masked MHA 仿真模式，避免索引器开销比例放大。

##### 8. 与 NSA 的对照 & 协同建议
| 维度 | NSA | DSA | 协同潜力 |
| ---- | ---- | ---- | ---- |
| 稀疏粒度 | Block + 选择集合 + 窗口 | 直接 Token 级 Top-k | 两级（先块后 Token）|
| 组件数 | 3 分支融合 | 2 阶段（索引 → 稀疏注意） | NSA 压缩/窗口 + DSA 精细选择 |
| 架构绑定 | 抽象，可配多注意力实现 | 绑定 MLA(MQA) | 共享潜在 KV |
| 复杂度降幅 | 多来源访存裁剪 | 主注意力 FLOPs 下砍 | 叠加：访存 + FLOPs 双减 |
| 训练策略 | 原生稀疏预训练 | 稠密热身 + 稀疏对齐 | 可共享 warm-up 模板 |
| 工程优化 | 分支融合 Kernel | FP8 索引器 + Top-k Gather | 联合调度 Pipeline |

组合路径示例：
1. Long Prefill：MLA +（可选）NSA 压缩分支 → 降 KV 建立成本；
2. Decode：先 NSA 过滤块 / 最近窗口保底，再 DSA Token 级 Top-k 精细筛选；
3. 叠加 MTP：减少“步数 × 每步 FLOPs”。

##### 9. 小结（DSA 相对 NSA 的“深入”）
1. 粒度更细：块级 → Token 级实时检索；
2. 迁移更平滑：双阶段 KL 对齐降低能力震荡；
3. 工程更极致：FP8 + 少头 + 低维 + MQA 共享；
4. 长上下文收益显著：32K~128K 区间加速 / 降本明显；
5. 组合更开放：可与 NSA / MLA / MTP / Flash / 量化叠加形成系统栈。

> 一句话总结：DSA 把 NSA“结构性稀疏 + 动态选择”的思想推进到更细粒度、更强工程可行性的 Token 级索引 + 稀疏注意力，实现长上下文端到端效率与性能的再平衡。
